...
(function() {
  ...
  // =========================================
  //   时区 & 今日日期（动态获取，跨午夜自动刷新）
  // =========================================
  let _timezoneOffset = 0; // 用户选择的时区偏移（分钟），0=系统默认
  let _cachedTodayDate = null;
  let _cachedTodayKey = '';

  /** 获取当前日期对象（根据用户选择的时区） */
  function getToday() {
    if (!_cachedTodayDate) _refreshTodayCache();
    return _cachedTodayDate;
  }

  /** 获取今日键值 'YYYY-M-D' */
  function getTodayKey() {
    if (!_cachedTodayKey || !_cachedTodayDate) _refreshTodayCache();
  ...
  function _refreshTodayCache() {
    ...
    if (_timezoneOffset !== 0) {
      ...
      now = new Date(utc + (_timezoneOffset * 60000));
    }
  ...
  let settings = {
    ...
    timezoneOffset: 0,    // 时区偏移（分钟），0=跟随系统
  ...
  const I18N = {
    ...
    'en-US': {
      ...
      mcpConfig: 'Configure MCP',
      generateMcpCode: '🔗 Generate Config',
  ...
  function applySettings() {
    ...
    // 任务显示行数（0=无限制，1/2/3=固定行数，-1=自适应行数）
    if (settings.taskLineClamp > 0) {
      document.documentElement.style.setProperty('--task-line-clamp', settings.taskLineClamp);
      document.documentElement.style.setProperty('--task-text-display', '-webkit-box');
      document.documentElement.style.setProperty('--task-text-overflow', 'hidden');
    } else {
      // 无限制 或 自适应：不使用 line-clamp 截断
      document.documentElement.style.setProperty('--task-line-clamp', 'unset');
      document.documentElement.style.setProperty('--task-text-display', 'block');
      document.documentElement.style.setProperty('--task-text-overflow', 'visible');
    }
    ...
    syncSelectValue('timezone-select', settings.timezoneOffset || 0);
    // 应用时区到全局变量
    _timezoneOffset = (settings.timezoneOffset || 0) * 1;
    ...
    syncCheckbox('auto-fit-font-check', settings.autoFitFont);
  ...
  //   年月快速选择器
  // =========================================
  let _pickerSelectedYear = currentYear;
  let _pickerSelectedMonth = currentMonth;

  function openMonthPicker() {
    _pickerSelectedYear = currentYear;
    _pickerSelectedMonth = currentMonth;

    var modal = document.getElementById('month-picker-modal');
    modal.classList.remove('hidden');

    // 填充年份列表
    var yearScroll = document.getElementById('year-scroll');
    yearScroll.innerHTML = '';
    var todayYear = getToday().getFullYear();
    var startYear = todayYear - 10;
    var endYear = todayYear + 10;

    // 上下留白
    var spacerTop = document.createElement('div');
    spacerTop.className = 'picker-spacer';
    yearScroll.appendChild(spacerTop);

    for (var y = startYear; y <= endYear; y++) {
      var item = document.createElement('div');
      item.className = 'picker-item' + (y === currentYear ? ' selected' : '');
      item.textContent = y + t('yearSuffix');
      item.dataset.value = y;
      item.addEventListener('click', (function(val, el) {
        return function() {
          _pickerSelectedYear = val;
          yearScroll.querySelectorAll('.picker-item').forEach(function(i) { i.classList.remove('selected'); });
          el.classList.add('selected');
          yearScroll.dataset.selected = val;
        };
      })(y, item));
      yearScroll.appendChild(item);
    }
    var spacerBottom = document.createElement('div');
    spacerBottom.className = 'picker-spacer';
    yearScroll.appendChild(spacerBottom);
    yearScroll.dataset.selected = currentYear;

    // 填充月份列表
    var monthScroll = document.getElementById('month-scroll');
    monthScroll.innerHTML = '';
    var monthNames = [t('jan'),t('feb'),t('mar'),t('apr'),t('may'),t('jun'),
                      t('jul'),t('aug'),t('sep'),t('oct'),t('nov'),t('dec')];

    var spacerTopM = document.createElement('div');
    spacerTopM.className = 'picker-spacer';
    monthScroll.appendChild(spacerTopM);

    for (var m = 0; m < 12; m++) {
      var mItem = document.createElement('div');
      mItem.className = 'picker-item' + (m === currentMonth ? ' selected' : '');
      mItem.textContent = monthNames[m];
      mItem.dataset.value = m;
      mItem.addEventListener('click', (function(val, el) {
        return function() {
          _pickerSelectedMonth = val;
          monthScroll.querySelectorAll('.picker-item').forEach(function(i) { i.classList.remove('selected'); });
          el.classList.add('selected');
          monthScroll.dataset.selected = val;
        };
      })(m, mItem));
      monthScroll.appendChild(mItem);
    }
    var spacerBottomM = document.createElement('div');
    spacerBottomM.className = 'picker-spacer';
    monthScroll.appendChild(spacerBottomM);
    monthScroll.dataset.selected = currentMonth;

    // 延迟滚动到选中项
    setTimeout(function() {
      scrollToSelected(yearScroll);
      scrollToSelected(monthScroll);
    }, 50);
  }

  function scrollToSelected(container) {
    var selected = container.querySelector('.picker-item.selected');
    if (selected) {
  ...
  function renderGrid() {
    ...
    if (isCurrentRealMonth) {
      ...
      startDate.setDate(startDate.getDate() - pastWs * 7 + weekOffset * 7);
    ...
    // 自适应字号：检测任务列表溢出，逐步缩小字号
    if (settings.autoFitFont) {
      applyAutoFitFont(grid);
    }
  }

  /** 自适应字号：检测每个单元格任务列表是否溢出，逐步缩小字号直到不溢出 */
  function applyAutoFitFont(grid) {
    const cells = grid.querySelectorAll('.day-cell');
    const baseFontSize = settings.fontSize || 16;
    const minFontSize = 9; // 最小字号
    cells.forEach(cell => {
      const taskList = cell.querySelector('.task-list');
      if (!taskList) return;
      // 只在有内容溢出时才缩小
      const taskItems = taskList.querySelectorAll('.task-item-cell .task-text');
      if (taskItems.length === 0) return;
      // 先重置为基准字号
      taskItems.forEach(el => el.style.fontSize = '');
      // 检测溢出：scrollHeight > clientHeight
      if (taskList.scrollHeight <= taskList.clientHeight) return;
      // 逐步缩小字号
      let currentSize = baseFontSize;
      while (currentSize > minFontSize && taskList.scrollHeight > taskList.clientHeight) {
        currentSize--;
        taskItems.forEach(el => el.style.fontSize = currentSize + 'px');
      }
    });
  }
  ...
  function createDayCell(cell, isWeekendCol) {
    ...
    var _t = getToday();
    const isToday = (year === _t.getFullYear() && month === _t.getMonth() && day === _t.getDate());
    ...
    if (day === 1) {
      ...
      monthLabel.className = 'month-label-badge';
    ...
    if (settings.showSolarterm && !holidayBadgeShown) {
  ...
  function bindEvents() {
    ...
    document.getElementById('prevMonth').addEventListener('click', () => {
      document.getElementById('calendar-grid').dispatchEvent(new WheelEvent('wheel', { deltaY: -1, bubbles: true }));
    });
    ...
    document.getElementById('todayBtn').addEventListener('click', () => {
      _refreshTodayCache();
      var _t = getToday();
      currentYear = _t.getFullYear();
      currentMonth = _t.getMonth();
      weekOffset = 0;
      renderCalendar();
    });

    // 年月标题双击打开快速选择器
    document.getElementById('header-center').addEventListener('dblclick', () => {
      openMonthPicker();
    });
    document.getElementById('month-picker-close').addEventListener('click', closeMonthPicker);
    document.getElementById('month-picker-overlay').addEventListener('click', closeMonthPicker);
    document.getElementById('month-picker-cancel').addEventListener('click', closeMonthPicker);
    ...
    document.getElementById('month-picker-ok').addEventListener('click', () => {
      var yr = parseInt(document.getElementById('year-scroll').dataset.selected) || currentYear;
      var mn = parseInt(document.getElementById('month-scroll').dataset.selected);
      if (!isNaN(mn)) {
        currentYear = yr;
        currentMonth = mn;
        weekOffset = 0;
    ...
    // 时区选择 — 改后立即刷新今日框和日历
    var _tzSel = document.getElementById('timezone-select');
    if (_tzSel) _tzSel.addEventListener('change', function() {
      _timezoneOffset = parseInt(this.value) || 0;
      settings.timezoneOffset = _timezoneOffset;
      saveSettings();
      // 强制刷新缓存（时区可能改变"今天"是哪一天）
      _cachedTodayKey = '';
    ...
    bindCheckboxChange('auto-fit-font-check', 'autoFitFont', renderCalendar);
...
`;
      ...
      // ===== MCP 配置代码生成 =====
      document.getElementById('mcp-config-btn').addEventListener('click', async () => {
        try {
          const appInfo = await window.electronAPI.getAppInfo();
          // exePath 形如 C:\...\release\win-unpacked\任务清单.exe
          // MCP Server 通过 extraResources 打包到 resources/mcp-server/
          const exeDir = appInfo.exePath.replace(/[/\\][^/\\]+$/, '');
          const mcpServerPath = exeDir.replace(/\\/g, '/') + '/resources/mcp-server/index.js';
          ...
          modal.className = 'about-modal-overlay';
          modal.innerHTML = ''
            + '<div class="about-modal-box" style="max-width:520px;">'
            + '  <div class="about-modal-header">'
            + '    <span>🔗 MCP 配置代码</span>'
            + '    <button class="about-modal-close" id="mcp-modal-close">✕</button>'
            + '  </div>'
            + '  <div class="about-modal-body" style="padding:16px 20px;">'
            + '    <p style="font-size:13px;color:var(--text-secondary);line-height:1.7;margin-bottom:12px;">'
            + '      将以下配置添加到 <b>WorkBuddy</b> 的 MCP 连接器中，即可让 AI 直接读写任务清单。<br>'
            + '      打开 WorkBuddy → 设置 → 自定义连接器 → 粘贴配置即可。'
            + '    </p>'
            + '    <div style="position:relative;background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;padding:12px;margin-bottom:12px;">'
            + '      <pre id="mcp-config-code" style="margin:0;font-family:Consolas,\'Courier New\',monospace;font-size:12.5px;line-height:1.6;white-space:pre-wrap;word-break:break-all;color:var(--text-primary);user-select:all;">' + _escHtml(configJson) + '</pre>'
            + '      <button id="mcp-copy-btn" style="position:absolute;top:8px;right:8px;background:var(--accent-color,#58a6ff);color:#fff;border:none;border-radius:4px;padding:4px 10px;font-size:11px;cursor:pointer;">复制</button>'
            + '    </div>'
            + '    <div style="font-size:12px;color:var(--text-secondary);line-height:1.7;">'
            + '      <p>📌 <b>使用前提：</b></p>'
            + '      <ul style="padding-left:16px;margin:4px 0 0;">'
            + '        <li>本机需安装 Node.js（v18+）</li>'
            + '        <li>首次使用前需在 mcp-server 目录执行 <code style="background:var(--bg-card);padding:1px 4px;border-radius:3px;">npm install</code></li>'
            + '        <li>MCP Server 路径：<code style="background:var(--bg-card);padding:1px 4px;border-radius:3px;word-break:break-all;">' + _escHtml(mcpServerPath) + '</code></li>'
            + '      </ul>'
            + '    </div>'
            + '  </div>'
            + '</div>';
          ...
          document.getElementById('mcp-modal-close').addEventListener('click', closeModal);
          modal.addEventListener('click', function(e) { if (e.target === modal) closeModal(); });

          // 复制按钮
          document.getElementById('mcp-copy-btn').addEventListener('click', async function() {
            var btn = this;
            try {
              const result = await window.electronAPI.copyToClipboard(configJson);
              if (result.success) {
                btn.textContent = '已复制 ✓';
                btn.style.background = '#4ade80';
                setTimeout(function() { btn.textContent = '复制'; btn.style.background = ''; }, 2000);
              } else {
                showToast('复制失败: ' + result.error);
              }
            } catch(e) {
      ...
      var ABOUT_CONTENTS = {
        ...
        changelog: {
          ...
          body: ''
            ...
            // v9.9.9 (2026-04-29) — 农历全月显示 + 年月快速选择器
            + '<h4 style="color:#58a6ff;">✨ v9.9.9 — 2026年4月29日（日历增强）</h4>'
            + '<ul style="line-height:1.8;padding-left:18px;font-size:12.5px;color:var(--text-secondary);">'
            + '  <li>🌙 <b>农历/节日全月显示</b>：非当月日期也显示农历、节日和节气</li>'
            + '  <li>📅 <b>年月快速选择器</b>：双击顶部年月标题，滚动快速跳转任意年月</li>'
            + '</ul>'

            // v9.9.8 (2026-04-29) — MCP 配置代码生成
            + '<h4 style="color:#58a6ff;">✨ v9.9.8 — 2026年4月29日（AI对接增强）</h4>'
            + '<ul style="line-height:1.8;padding-left:18px;font-size:12.5px;color:var(--text-secondary);">'
            + '  <li>🔗 新增「配置 MCP」功能，一键生成 MCP 连接器配置代码</li>'
            + '  <li>📦 内置 MCP Server，支持 AI 直接读写任务清单</li>'
            + '</ul>'

            // v9.9.7 (2026-04-29) — 滚轮按周翻页 + 每月1号月份标识
            + '<h4 style="color:#58a6ff;">✨ v9.9.7 — 2026年4月29日（交互优化）</h4>'
            + '<ul style="line-height:1.8;padding-left:18px;font-size:12.5px;color:var(--text-secondary);">'
            + '  <li>🖱️ 鼠标滚轮翻页改为按周滚动（每次翻1周，而非整月跳转）</li>'
            + '  <li>📅 每月1号日期旁显示"X月"标识，方便快速定位月份</li>'
            + '</ul>'
    ...
    try {
      ...
      function getCurrentVersion() {
        return new Promise(function(resolve) {
          // 优先从 package.json 硬编码（最可靠）
          var currentVersion = '9.9.3';
