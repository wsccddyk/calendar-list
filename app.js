...
`;
  ...
  function handleUpdateEvent(data) {
    logInfo('UPDATE', '收到事件', { state: data.state });
    ...
    switch (data.state) {
      ...
      case 'not-available':
        // 只有自动检查时才静默处理；手动检查由 .then() 回调处理
        if (!_checkingForUpdate) {
          // 自动检查 → 静默忽略
          logInfo('UPDATE', '自动检查: 已是最新版本');
        }
        break;
      ...
      case 'error':
        ...
        if (_checkingForUpdate) {
          _checkingForUpdate = false;
          if (data.message) showUpdateResultDialog(false, null, data.message);
          // 重置按钮
