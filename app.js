...
(function() {
  ...
  function applySettings() {
    ...
    syncSelectValue('language-select', 'zh-CN');
...
`;
      ...
      function _renderLogViewer() {
        ...
        _logSummaryBadge.textContent = summary.total + ' 条 | 保留 ' + LOG_MAX_DAYS + ' 天';

        if (logs.length === 0) {
          _logContainer.innerHTML = '<span style="color:#666;">（无匹配日志）</span>';
          return;
        }
      ...
      function showAboutModal(key) {
        ...
        aboutModalEl.innerHTML = ''
          ...
          + '  <div class="about-modal-header"><span>' + content.title + '</span><button class="about-modal-close">✕</button></div>'
  ...
  function handleUpdateEvent(data) {
    switch (data.state) {
      ...
      case 'downloaded':
        ...
        break;
      case 'error':
        if (data.message && data.message.indexOf('404') === -1) {
          showToast('更新检查失败：' + data.message, 3000);
        }
        break;
    }
  }
  ...
  function showUpdateDialog(info) {
    ...
    _updateModal.innerHTML = ''
      ...
      + '  <div class="about-modal-header"><span>🔄 发现新版本</span><button class="about-modal-close" id="update-close-btn">✕</button></div>'
      ...
      + '      <button class="action-btn" id="update-later-btn" style="padding:6px 16px;">稍后再说</button>'
      + '      <button class="action-btn primary-btn" id="update-download-btn" style="padding:6px 20px;background:var(--accent);">立即更新 ✨</button>'
      + '    </div>'
      ...
      + '      </div>'
      + '      <div id="update-progress-text" style="text-align:center;font-size:12px;color:var(--text-secondary);margin-top:4px;">准备下载...</div>'
      + '    </div>'
      + '  </div>'
      + '</div>';
