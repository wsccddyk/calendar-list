  ...
  function bindEvents() {
    ...
    if (btnCheckUpdateAbout) {
      btnCheckUpdateAbout.addEventListener('click', function() {
        ...
        _manualCheckStartTime = Date.now(); // 记录手动检查开始时间（用于过滤自动检查残留事件）
...
`;
  ...
  function init() {
    LogInfo('init', '=== 应用启动 ===');
    // 尝试同步获取版本号（不影响其他初始化逻辑）
    try {
      var syncVer = getVersionSync();
      if (syncVer) {
        _localVersion = syncVer;
        LogInfo('init', '同步获取版本号: ' + syncVer);
      }
    } catch(e) {
      LogWarn('init', '同步版本号获取失败（非致命）: ' + e.message);
    }
    // 全局错误捕获
  ...
  var _manualCheckStartTime = 0; // 手动检查开始时间戳，用于过滤启动自动检查的残留事件
  ...
  function handleUpdateEvent(data) {
    // 【v9.9.9 关键修复】手动检查期间，屏蔽自动检查的残留事件
    // 手动检查（updater-click）通过 .then()/ .catch() 回调自行处理所有结果弹窗，
    // 不需要 handleUpdateEvent 再弹一次。只有自动检查（启动后静默检查）才走这里。
    if (_checkingForUpdate) {
      // 下载进度类事件仍需处理（用于显示进度条）
      if (data.state === 'downloading' || data.state === 'downloaded') {
        // 继续处理进度
      } else {
        // not-available / error / available 等终态事件：忽略，由 .then() 回调处理
        logInfo('UPDATE', '手动检查模式：忽略事件（由IPC回调处理）', { state: data.state });
        return;
      }
    }
    ...
    switch (data.state) {
      ...
      case 'error':
        ...
        // 【v9.9.9】如果正在显示更新弹窗（_updateModal存在），说明是下载阶段出错，也需要报错
        if (_checkingForUpdate || _updateModal) {
          var wasChecking = _checkingForUpdate;
          _checkingForUpdate = false;
          logError('UPDATE', '收到错误事件', { message: data.message, wasManualCheck: wasChecking });
          if (_updateModal && !wasChecking) {
            // 下载阶段出错 → 在弹窗内显示错误
            var text2 = document.getElementById('update-progress-text');
            var btn2 = document.querySelector('#update-download-btn');
            if (text2) text2.textContent = '❌ 下载失败：' + (data.message || '未知错误');
            if (btn2) { btn2.disabled = false; btn2.textContent = '🔄 重试'; }
          } else if (data.message) {
            showUpdateResultDialog(false, null, data.message);
          }
          // 重置按钮状态
          var btn3 = document.getElementById('btn-check-update-about');
          var st = document.getElementById('update-status-text');
          if (btn3) { btn3.disabled = false; btn3.textContent = t('updateCheck'); }
          if (st) { st.textContent = ''; st.style.animation = ''; }
        } else {
          logError('UPDATE', '自动检查失败（静默）', { message: data.message });
        }
        break;
    }
  }
  ...
  function showUpdateDialog(info) {
    ...
    _updateModal.querySelector('#update-download-btn').addEventListener('click', function() {
      ...
      document.getElementById('update-progress-text').textContent = '正在连接服务器...';
      _checkingForUpdate = false; // 【v9.9.9】下载开始时重置标记，确保后续进度/完成/错误事件不被屏蔽
      logInfo('UPDATE', '用户点击立即更新，开始下载');
      if (window.electronAPI && window.electronAPI.updaterDownload) {
        window.electronAPI.updaterDownload().then(function(result) {
          logInfo('UPDATE', 'downloadUpdate 返回:', JSON.stringify(result));
        }).catch(function(err) {
          logError('UPDATE', 'downloadUpdate 失败:', err);
          var text = document.getElementById('update-progress-text');
          var btn = document.querySelector('#update-download-btn');
          if (text) text.textContent = '❌ 下载失败：' + (err.message || err || '未知错误');
          if (btn) { btn.disabled = false; btn.textContent = '🔄 重试'; }
        });
      } else {
        logError('UPDATE', 'electronAPI.updaterDownload 不存在！');
      }
    });
  ...
  function updateDownloadProgress(percent) {
    ...
    var text = document.getElementById('update-progress-text');
    logInfo('UPDATE', '下载进度: ' + percent + '%');
    if (bar) bar.style.width = percent + '%';
    if (text) text.textContent = '下载中... ' + Math.round(percent) + '%';
  }

  function updateDownloadComplete(version) {
    var text = document.getElementById('update-progress-text');
    var btn = document.querySelector('#update-download-btn');
    logInfo('UPDATE', '下载完成，版本: ' + version);
  ...
  // ==========================================
  /**
   * 同步获取版本号（顶层函数，可在 init() 中调用）
   * 优先级：sync IPC > 缓存值 > null
   */
  function getVersionSync() {
    try {
      if (window.electronAPI && window.electronAPI.getVersionSync) {
        var v = window.electronAPI.getVersionSync();
        if (v) return v;
      }
    } catch(e) {}
    if (_localVersion && _localVersion !== 'unknown') return _localVersion;
    return null;
  }
  ...
  function checkAndShowChangelog() {
    try {
      ...
      getCurrentVersion().then(function(currentVersion) {
        // 如果异步返回了有效版本，更新缓存；否则保留同步版本
        if (currentVersion && _localVersion !== currentVersion) {
          _localVersion = currentVersion;
        }
        // 确保有可用版本号
        var ver = _localVersion || 'unknown';
        
        var lastVersion = localStorage.getItem(LAST_VERSION_KEY);
        LogInfo('changelog', '当前版本: ' + ver + ', 上次版本: ' + (lastVersion || '(无)'));
        ...
        if (verEl) {
          ...
          verEl.textContent = 'v' + ver + ' (' + dateStr + ')';
        }

        // 版本号变化说明刚完成更新（首次安装或更新后）
        if (lastVersion && lastVersion !== ver) {
          LogInfo('changelog', '检测到版本变化 (' + lastVersion + ' → ' + ver + ')，将展示更新日志');
          // 延迟展示，等界面渲染完毕后弹出
          setTimeout(function() {
